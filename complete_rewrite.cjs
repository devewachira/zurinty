...
const pages = [
  { name: 'DroneReel', title: 'Commercial Drone Reel' },
  { name: 'DealerNetwork', title: 'Dealer Network' },
  { name: 'BrandSystem', title: 'Brand System' },
  { name: 'PortfolioTemplate', title: 'Portfolio Template' },
  { name: 'CRELandingPage', title: 'CRE Landing Page' },
  { name: 'SavannaCoffee', title: 'Savanna Coffee Roasters' },
  { name: 'NexusTech', title: 'Nexus Tech Solutions' },
  { name: 'BloomWellness', title: 'Bloom Wellness Spa' },
  { name: 'PeakAdventure', title: 'Peak Adventure Co.' },
  { name: 'CoastalRealty', title: 'Coastal Realty Group' },
  { name: 'EmberCreative', title: 'Ember Creative Studio' },
  { name: 'HarvestKitchen', title: 'Harvest Kitchen & Bar' },
  { name: 'AtlasFinancial', title: 'Atlas Financial Partners' },
  { name: 'VerdeOrganics', title: 'Verde Organics' },
  { name: 'SkylineArchitecture', title: 'Skyline Architecture' }
];

const template = (name, title) => `import HeroSection from "../components/trellis/HeroSection";
...
const ${name} = () => {
  return (
    <div className="min-h-screen bg-white text-[#1e1e3f] selection:bg-[#32a386] selection:text-white overflow-hidden font-sans">
      ...
      <HeroSection title="${title}" image="/assets/img/portfolio/trellis-macbook.png" />
      <ProjectInfo 
      ...
      <ImageShowcase images={["/assets/img/portfolio/trellis-1.png", "/assets/img/portfolio/trellis-2.png", "/assets/img/portfolio/trellis-3.png"]} />
      ...
      <section className="bg-white py-24 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            <div className="rounded-[2.5rem] overflow-hidden shadow-3xl bg-white transition-all duration-700 hover:scale-[1.01] aspect-video">
              <img src="/assets/img/portfolio/trellis-7.png" alt="Feature 1" className="w-full h-full object-contain bg-gray-50" />
            </div>
            <div className="rounded-[2.5rem] overflow-hidden shadow-3xl bg-[#0b1b2a] transition-all duration-700 hover:scale-[1.01] aspect-video">
              <img src="/assets/img/portfolio/trellis-8.png" alt="Feature 2" className="w-full h-full object-contain" />
            </div>
          </div>
        </div>
      </section>
...
pages.forEach(p => {
  fs.writeFileSync(`src/pages/${p.name}.tsx`, template(p.name, p.title));
});
